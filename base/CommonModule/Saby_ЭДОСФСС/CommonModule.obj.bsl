...
Функция ПолучитьСтруктуруОтправляемогоКомплекта(ПараметрыСообщения)
	...
	НашаОрганизация = МодульЭДОСФССПовтИсп().Saby_НашаОрганизация(ПараметрыСообщения.Организация);
...
Функция ВыполнитьINI(algorithm, endpoint, Параметры)
	...
	ПараметрыКоманды = Новый Структура("algorithm, endpoint, params", algorithm, endpoint);
	Для Каждого Парам Из Параметры Цикл
		set_prop(Параметры, ПараметрыКоманды, Парам.Ключ, Парам.Ключ);
	КонецЦикла;
...
Процедура ЗагрузитьМЧДСФРПоОрганизации(Организация) Экспорт
	...
	Пока Истина Цикл
		...
		РезультатСписокДокументов = ПолучитьСписокДокументовМЧД(Организация);
...
Функция ПолучитьСписокДокументовМЧД(Организация) Экспорт
