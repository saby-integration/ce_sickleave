...
Функция ОбработатьСписокИзменений(Контекст) 
	...
	Иначе
		...
		Если context_params = Неопределено Тогда
			ИнфОбОшибке = Новый Структура("Описание", "Для выполнения действия необходимо авторизоваться в SABY.");
			ДобавитьОшибкуПолученияСообщенийФСС(Контекст, ИнфОбОшибке);
...
Процедура ПолучитьСписокИзмененийЦикл(Контекст) 
	...
	Если Контекст.ЕстьЕще <> "Да" Тогда
		Если Не Контекст.Свойство("ДатаСообщенийПо") И Контекст.Фильтр.Свойство("ИдентификаторСобытия") Тогда
...
Процедура ОбработатьСписокИзмененийЦикл(Контекст) 
	...
	Для Каждого Документ Из СписокИзменений Цикл
		...
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ДобавитьОшибкуПолученияСообщенийФСС(Контекст, ИнфОбОшибке);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ИнфОбОшибке.ИмяМодуля + " " + ИнфОбОшибке.ИсходнаяСтрока + " " + ИнфОбОшибке.Описание);
