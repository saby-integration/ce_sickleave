...
Процедура ОтправитьСообщениеСЭДОПослеПодписанияЗапроса(РезультатПодписиШифрования, Контекст) Экспорт 
	...
	Попытка
		РезультатОтправки = МодульЭДОСФСС().ОтправитьСообщениеСЭДОЧерезAPIСБИС(ПараметрыСообщения);
	Исключение
...
Процедура ПолучитьСообщенияСЭДОЗаПериодПоОрганизацииЦикл(Результат, Контекст) Экспорт
	...
	Попытка
		СвойстваОрганизации = МодульЭДОСФССПовтИсп().СвойстваОрганизацииДляОбмена(Контекст.Организация);
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
...
Процедура ПолучитьСписокСлужебныхЭтаповЦикл(Контекст) 
	...
	КонецЕсли;
	Попытка
		СписокСлужебныхЭтапов = МодульЭДОСФСС().СписокСлужебныхЭтаповПоДокументамСЭДО(Контекст.context_params, Контекст.Фильтр);
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке(); 
		ДобавитьОшибкуПолученияСообщенийФСС(Контекст, ИнфОбОшибке);
		СообщитьОбОшибке(ИнфОбОшибке);
...
Процедура ОбработатьСписокСлужебныхЭтаповЦикл(Контекст) 
	...
	Если Документ["Тип"] <> "ДокументСЭДО" И Документ["Тип"] <> "DocumentSEDI" Тогда
		Попытка
			МодульЭДОСФСС().ОтложитьСлужебныйЭтап(Документ);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ДобавитьОшибкуПолученияСообщенийФСС(Контекст, ИнфОбОшибке);
		КонецПопытки;
	...
	Иначе
		...
		Попытка
			МодульЭДОСФСС().ПодготовитьВыполнитьДействие(Документ, ПараметрыСообщения);
		Исключение
			ИнфОбОшибке = ИнформацияОбОшибке();
			ДобавитьОшибкуПолученияСообщенийФСС(Контекст, ИнфОбОшибке);
		КонецПопытки;
...
Процедура ОпределитьТипИПодписатьСлужебныеДокументы(Документ, Контекст)
	...
	Попытка
		...
		ПараметрыСообщения.Вставить("РегистрационныйНомерФСС", СвойстваОрганизации.РегистрационныйНомерФСС);
		ПараметрыСообщения.Вставить("СертификатСтрахователяОтпечаток", СвойстваОрганизации.СертификатСтрахователяОтпечаток);
		РезультатПодготовкиДействия = МодульЭДОСФСС().ПодготвитьДействие(Документ, ПараметрыСообщения);
...
Процедура ПодписатьЗапросСлужебногоЭтапаПослеПодписанияЗапроса(РезультатПодписиШифрования, Контекст) Экспорт 
	...
	Попытка
		//РезультатВыполненияДействия = МодульЭДОСФСС().ПодготовитьВыполнитьДействие(Контекст.Документ, Контекст.ПараметрыСообщения);
		...
		МодульЭДОСФСС().ВыполнитьДействие(РезультатПодготовкиДействия, Контекст.ПараметрыСообщения);
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
		ДобавитьОшибкуПолученияСообщенийФСС(Контекст, ИнфОбОшибке);
	КонецПопытки;
...
Процедура РасшифроватьОтветСлужебногоЭтапа(Документ, Контекст)
	...
	РезультатПодготовкиДействия = МодульЭДОСФСС().ПодготвитьДействие(
		Документ, 
		ПараметрыСообщения
	);
...
Процедура РасшифроватьОтветСлужебногоЭтапаЦикл(Контекст)
	...
	Если Контекст.Итератор > Контекст.Документ["Этап"][0]["Вложение"].ВГраница() Тогда
		...
		Иначе
			Попытка
				МодульЭДОСФСС().ВыполнитьДействие(Контекст.Документ, Новый Структура);
			Исключение
				ИнфОбОшибке = ИнформацияОбОшибке();
				ДобавитьОшибкуПолученияСообщенийФСС(Контекст, ИнфОбОшибке);
			КонецПопытки;
	...
	СообщениеФССXML = МодульЭДОСФСС().ЗагрузитьСообщениеФССXMLИзСбис(
...
#КонецОбласти
	МассивОбрабатываемыхПодтиповВложений = МодульЭДОСФСС().ВыполнитьINI(
...
Процедура ПолучитьСписокИзмененийЦикл(Контекст) 
	...
	Попытка
		СписокИзменений = МодульЭДОСФСС().СписокИзмененийПоДокументамСЭДО(Контекст.context_params, Контекст.Фильтр);
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
		ДобавитьОшибкуПолученияСообщенийФСС(Контекст, ИнфОбОшибке);
		СообщитьОбОшибке(ИнфОбОшибке);
...
Процедура ОбработатьСписокИзмененийЦикл(Контекст) 
	...
	Если КоличествоИзменений > 0 Тогда
		...
		Если Не Контекст.Свойство("ДатаСообщенийПо") Тогда
			...
			МодульЭДОСФСС().ЗаписатьНастройкиПодключения("sickleave_last_event", Контекст.context_params.sickleave_last_event);
		КонецЕсли;
	КонецЕсли;
	ПолучитьСписокИзмененийЦикл(Контекст);
КонецПроцедуры
...
Процедура ОбработатьСобытиеДокумента(Документ, Организация, Контекст)
	...
	Если Документ["Событие"][0]["Вложение"][0]["Подтип"] = "описаниеОшибки" Тогда
		Результат = МодульЭДОСФСС().ЗаписатьУведомлениеОбОшибке(Документ["Редакция"][0]["Идентификатор"], Документ);
		Если Результат <> Неопределено Тогда
	...
	Если Документ["Событие"][0]["Вложение"][0]["Подтип"] = "SEDI_UUID" Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеОбработки1СSEDI_UUID", МодульЭДОСФССКлиент(), Контекст);
	...
	Иначе
		Если Документ["Расширение"]["ПодТипКомплекта"] = "SEDI_NOTICE" Тогда
			Если Документ["Событие"][0]["Название"] = "подтверждение"
				...
				МодульЭДОСФСС().СохранитьДатуОтправкиПодтвержденияОПрочтении(Организация, ИдентификаторыСообщений);
			КонецЕсли;
		КонецЕсли;

		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеОбработкиИзменения", МодульЭДОСФССКлиент(), Контекст);
...
Процедура ПослеОбработки1СSEDI_UUID(Результат, Контекст) Экспорт
	...
	МодульЭДОСФСС().ЗаписатьSEDI_UUID(Результат.ИдентификаторЗапроса, Контекст.Документ);
...
Функция СформироватьТекстОшибки(ИнфОбОшибке)
	ОшибкаСтруктура = МодульЭДОСФСС().NewExtExceptionСтруктура(ИнфОбОшибке);
