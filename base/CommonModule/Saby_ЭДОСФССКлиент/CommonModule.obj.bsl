...
#Область include_sickleave_base_CommonModule_ИмяФункцииМассивОбрабатываемыхПодтиповВложений
#КонецОбласти
	МассивОбрабатываемыхПодтиповВложений = Sedo_ЭДОСФСС.ВыполнитьINI(
		"Blockly_СформироватьКомплектПроактивногоБольничного_Other",
		ИмяФункцииМассивОбрабатываемыхПодтиповВложений,
		Новый Структура);
	...
	Иначе
		Если Документ["Расширение"]["ПодТипКомплекта"] = "SEDI_NOTICE" Тогда
			Если Документ["Событие"][0]["Название"] = "подтверждение"
					...
					// Надо обязательно сохранить дату отправки подтверждения о прочтении сообщения (Дата отправки сообщения с типом 6),
